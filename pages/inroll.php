...
">
   ...
   <!--Start Tabs Container-->
    ...
    <ul class="nav nav-tabs mb-1 d-flex justify-content-around align-items-cente" id="myTab0" role="tablist">
      ...
      <li class="nav-item d-none" role="presentation" style="width:25%">
        <button class="nav-link w-100 d-flex justify-content-center align-items-center" id="profile-tab0" data-mdb-toggle="tab" data-mdb-target="#profile0" type="button" role="tab" aria-controls="profile" aria-selected="false" style="border-radius:0px;">
          Test
        </button>
      </li>
...
{
  $course_section =  $course_data['section'];
}
...
echo '<div class="card-header py-1 bg-dark text-light" id="headingOne">
                   <h5 class="mb-0">
                     <div class="border-0 d-flex justify-content-between align-items-center py-2" data-toggle="collapse" data-target="#collapse" aria-expanded="true" aria-controls="collapse" style="font-size:15px;">
                       <b class="d-flex justify-content-around align-items-center">'.$course_section.'</b>
...
// 
   ...
   echo '<span class="per_page" per='.$result_per_page.'></span>';
      ...
      // Lecture query
         echo '<div class="lecture-box" style="height:560px;">
  ...
  //  echo $this_page_first_result = ($page-1)*$result_per_page;
     ...
     // lecture 
     echo'<div class="w-100 py-3 d-flex justify-content-center align-items-center flex-wrap">';
    for($page=1;$page<=$number_of_pages;$page++){
      ...
      for ($x = 1; $x <= 9; $x++) {
       ...
       echo '<button class="btn btn-sm mx-1 mb-1 limit-btn" id="limit-ntn-'.$page.'" limit='.$page.' style="border:1px solid black;">'.$page.'</button>';
    }
...
<?php  
    // Total comment
    $total_comment = "SELECT COUNT(query_message) AS comment_no FROM user_query WHERE course_id='$course_id'";
...
<!--Start Tabs Container-->
    ...
    <div class="col-sm-12 mt-2 mt-md-4 col-md-6 video_query_container pb-3" style="position:relative;border-bottom:2.5px solid #ddd;background:#FEF8EF;">
      
    <!-- Video link  -->
    <?php 
      $course_first_lectre = "SELECT * FROM lecture WHERE course_id='$course_id'";
    ...
    ?>
      ...
      <div class="w-100 <?php echo $buy_btn; ?> pt-2 buy_like_box" style="height:70px;">
         <a class="w-100 buy-btn" href="#" course_id="<?php echo  $_SESSION['course_click']; ?>" ><div class="h-100 text-light w-100 mr-2 d-flex justify-content-center align-items-center" style="width:80%;background:#8710D8;"><b>BUY NOW RS : <?php echo $course_prize ; ?></b></div></a> 
          <div class="h-100 bg-info d-flex justify-content-center align-items-center like-icon text-dark" style="width:20%;cursor:pointer;"><i class="fa fa-heart" aria-hidden="true"></i></div>
      </div>

     

      <button type="button" name="add_review" id="add_review" class="btn mt-3 shadow-none text-capitalize text-light text-bold <?php echo "d-none"; ?>" data-toggle="modal" data-target="#start_review_modual" style="background-color:#006CA8;">
         <i class="fa fa-star pr-2"></i> <span class="pl-2">Rate Now</span>
      </button>
...
<div class="modal fade" id="review_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color:#006CA8;">
        <h5 class="modal-title text-light" id="exampleModalLongTitle">Course Review</h5>
        <button type="button" id="close_review_modual" class="close" data-dismiss="modal" aria-label="Close">
      ...
      <div class="modal-body">
	        	...
	        	<div class="form-group">
	        		<textarea name="user_review" id="user_review" class="form-control shadow-none border-none" placeholder="Type Review Here" style="outline:none;border-color:none;"></textarea>
              <span class="text-danger d-none ml-3 revitw-notice">Write Your Review.</span>
	        	</div>
	        	<div class="form-group text-center mt-4">
	        		<button type="button" class="btn shadow-none text-light px-5 text-capitalize post-review-btn" id="save_review" style="background-color:#006CA8;font-size:15px;">Post Review</button>
              <div class="alert alert-success text-bold d-none review-message-box" role="alert">
                 Thanks For Review Our Course.
              </div>
	        	</div>
...
$(document).ready(function(){
  ...
  $(document).ready(function(){
    ...
    //   if ((width >= 768  )) {
    //   var  per_pages_lecture = "15";
    //   }
    //   else{
    //   var  per_pages_lecture = "10";
    //   }
...
$('#save_review').click(function(){
    ...
    if(user_review == '')
    {
        $(".revitw-notice").removeClass("d-none");
        
        setTimeout(function(){
          $(".revitw-notice").addClass("d-none"); 
    ...
    else
    {
       ...
       var user_name =  $("#user_span").attr("full_name"); 
        ...
        $.ajax({
            ...
            {
              $(".review-message-box").removeClass("d-none");
              $(".review-message-box").html(data);
              $(".post-review-btn").addClass("d-none"); 
                setTimeout(function(){
                  // $('#review_modal').modal('hide');
                  // $(".review-message-box").addClass("d-none");
                  $(".post-review-btn").removeClass("d-none"); 
                },2000);
...
$(document).ready(function(){
  $("#close_review_modual").click(function(){
     $('#review_modal').modal('hide');
  });
});


// window load get review and star 
load_rating_data();
