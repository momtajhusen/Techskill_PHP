  ...
  if(!empty($data['given_name']) && !empty($data['family_name']) && !empty($data['email']) && !empty($data['picture']))
  {
    ...
    $_SESSION['user_email_address'] = $user_email;
           ...
           if ($result->num_rows == 0){
                ...
                // insert new user 
                $signup = "INSERT INTO signup_user (first_name, last_name, email, user_pic, verify_status, verify_code)
                VALUES ('$user_first_name', '$user_last_name', '$user_email', '$user_picture', 'Active', '293020')";
